  ...
  <div class="me-view-comment-item">
    ...
    <div>
      ...
      <div class="me-reply-list">
        <div class="me-reply-item" v-for="c in comment.cauldrons" :key="c.id">
    ...
    methods: {
      showComment(commentShowIndex, toUser) {
        ...
        if(!Object.keys(that.toUsers).length==0) {
            if (that.commentShowIndex !== commentShowIndex) {//严重耦合代码，后期等我有能力在逐渐改善
                if (toUser) {
                    if (toUser.account==that.toUsers.account){
                        that.$message({type: 'error', message: '不能回复自己，请注意·', showClose: true})
                        this.commentShow = false
                        this.commentShowIndex = ''
                    }else {
                        that.placeholder = `@${toUser.nickname} `
                        that.reply.toUser = toUser
           ...
           that.$message({type: 'error', message: '您还未登录，请登录·', showClose: true})
        }
      },
      publishComment() {
        let that = this
        ...
        that.toUsers=that.$store.getters.getUser
        if(!Object.keys(that.toUsers).length==0){
            that.reply.author=that.toUsers
            console.log(that.reply.author)
        }
        that.$api.api.publishComment(that.reply).then(data => {
          that.$message({type: 'success', message: '评论成功', showClose: true})
          if(!that.comment.cauldrons){
            that.comment.cauldrons = []
          }
          that.comment.cauldrons.unshift(data.data)
