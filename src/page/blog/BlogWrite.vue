      ...
      <el-dialog title="摘要 分类 标签"
        ...
        <el-form :model="articleForm" ref="articleForm" :rules="rules">
          ...
          <el-form-item label="文章标签" prop="tags">
            <el-checkbox-group v-model="articleForm.label">
              <el-checkbox v-for="t in label" :key="t.id" :label="t.id" name="tags">{{t.tagname}}</el-checkbox>
            </el-checkbox-group>
          </el-form-item>
        </el-form>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getArticleById(id) {
        let that = this
        that.$api.api.getArticleByIds("blog/"+id).then(data => {
          Object.assign(that.articleForm, data.data)
          that.articleForm.editor.value = data.data.body.content
          let tags = this.articleForm.label.map(function (item) {
            return item.id;
          })
          this.articleForm.label = tags
        }).catch(error => {
          if (error !== 'error') {
      ...
      publish(articleForm) {
        ...
        this.$refs[articleForm].validate((valid) => {
          if (valid) {
            let label = this.articleForm.label.map(function (item) {
              return {id: item};
            });
            let author= that.$store.getters.getUser;
            let article = {
              id: this.articleForm.id,
              title: this.articleForm.title,
              synopsis: this.articleForm.summary,
              category:{id:this.articleForm.category.id},
              // this.articleForm.category.id,
              author:{id:author.id},
              label:label,
              body: {
            ...
            let loading = this.$loading({
              ...
              console.log(article)
              that.$api.api.publishShows(article).then((data) => {
              loading.close();
      ...
      getCategorysAndTags() {
        let that = this
        that.$api.api.getAllCategorys().then(data => {
        ...
        that.$api.api.getAllTags().then(data => {
        that.label = data.data
        }).catch(error => {
          if (error !== 'error') {
